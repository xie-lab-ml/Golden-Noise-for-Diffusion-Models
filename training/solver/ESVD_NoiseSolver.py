...
from model import NoiseUnet, SVDNoiseUnet, SVDNoiseUnet_wo_attention
from reward_model.eval_pickscore import PickScore
...
class SVD_Embedding_Solver:
    def __init__(
        ...
        # TODO: make ablation for svd unet without self-attention
        self.unet_svd = SVDNoiseUnet_wo_attention(resolution=config.size//8).to(device).to(torch.float32)
        ...
        for epoch in range(1, total_epoch + 1):
            ...
            for step, (original_noise, optimized_noise, prompt) in enumerate(pbar, 1):
                # with self.accelerator.accumulate([self.unet_svd, self.text_embedding, self.text_embedding, self._alpha, self._beta]):
                    ...
                    ) = self.pipeline.encode_prompt(prompt=prompt[0], device=self.device) # prompt[0]
                    ...
                    # TODO
                    # replace the text embedding with random noise to verify the effect of prompt embeds
                    # prompt_embeds = torch.rand_like(prompt_embeds).to(self.device)
                    ...
                    # encoder_hidden_states_embedding = original_noise
                    
                    golden_embedding = self.unet_embedding(encoder_hidden_states_embedding.float())

                    # golden_noise = self.unet_svd(encoder_hidden_states_svd.float()) + self._beta * golden_embedding
    ...
    def generate(self,
        ...
        if config.pipeline == 'SD2.1':
            ...
            (
                ...
                pooled_prompt_embeds,
                negative_pooled_prompt_embeds,
            ) = self.pipeline.encode_prompt(prompt=prompt, device=self.device) # prompt[0]
        ...
        # encoder_hidden_states_embedding = latent
        ...
        # merge latent
        # golden_noise = golden_noise + (golden_noise - latent)

        if config.pipeline == 'AYS':
            ...
            print("Successfully load new pipeline!")
        ...
        elif config.pipeline == 'LCM' or config.pipeline == 'PCM' or config.pipeline == 'SDXL-Lightning':
            original_img = self.pipeline(
                      prompt=prompt, 
        ...
        if save_postfix is not None:
            # golden_img.save(f'/home/zhouzikai/NoiseModel/denoising-optimization-for-diffusion-models-main/results/visualization/SDXL-50-draw/golden_img_news_{save_postfix}.png')
