...
class SVD_Solver:
    ...
    def train(
        ...
        # self.unet, self.text_embedding, self._alpha, self.optimizer, train_loader = self.accelerator.prepare(self.unet, self.text_embedding, self._alpha, self.optimizer, train_loader)
        ...
        for epoch in range(1, total_epoch + 1):
            ...
            for step, (original_noise, optimized_noise, prompt) in enumerate(pbar, 1):
                # with self.accelerator.accumulate([self.unet, self.text_embedding, self._alpha]):
                
                # original_noise += 0.001 * torch.randn(original_noise.shape[0], 4, 128, 128)
